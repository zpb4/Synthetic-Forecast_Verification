...
Q_ref,dowy_ref = syn_util.extract_obs(ref_st, ref_end, Rsyn_path=syn_path1,loc=loc,site=site)
Qf_ref_ens_inp = verify.create_obs_ref_ens(Q_ref, dowy_ref, sd, ed)
Qf_ref_ens = Qf_ref_ens_inp[val_idx,:]
...
np.savez_compressed('data/%s-%s_ecrps-ss_syn-forecast%s_nsamp=%s.npz' %(loc,site,syn_vers1+syn_vers1_param,nsamps), arr=ecrps_ss_v1)
np.savez_compressed('data/%s-%s_ecrps-ss_syn-forecast%s_nsamp=%s.npz' %(loc,site,syn_vers2+syn_vers2_param,nsamps), arr=ecrps_ss_v2)

np.savez_compressed('data/%s-%s_ecrps-ss_hefs.npz', arr=ecrps_ss_hefs)
