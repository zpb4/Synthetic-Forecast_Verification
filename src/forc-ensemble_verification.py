...
ax1.set_ylabel('Error ($\sqrt{mse}$)',fontsize='large')
...
ax3.set_xlabel('Spread ($\sqrt{var}$)',fontsize='large')
ax3.set_ylabel('Error ($\sqrt{mse}$)',fontsize='large')
...
ax4.set_xlabel('Spread ($\sqrt{var}$)',fontsize='large')
#plt.ylabel('Error ($\sqrt{mse}$)')
ax4.yaxis.set_ticklabels([])
...
ecrps_ss_v1_inp = np.load('data/%s-%s_ecrps-ss_syn-forecast%s_nsamp=%s.npz' %(loc,site,syn_vers1+vers_out1,nsamps))['arr']
ecrps_ss_v2_inp = np.load('data/%s-%s_ecrps-ss_syn-forecast%s_nsamp=%s.npz' %(loc,site,syn_vers2+vers_out2,nsamps))['arr']
ecrps_ss_hefs_inp = np.load('data/%s-%s_ecrps-ss_hefs.npz' %(loc,site))['arr']
...
ax1.set_ylabel('eCRPS skill score',fontsize='large')
#ax1.text(.95,.75,str(low_pcnt_ecrps[0])+'-'+str(low_pcnt_ecrps[1])+' percentile',fontsize='large',transform=ax1.transAxes,ha='right')
ax1.text(.95,.75,'lower '+str(int(round((low_pcnt_ecrps[1])*100,0)))+'% flows',transform=ax1.transAxes,ha='right',fontsize='large')
...
ax2.text(.95,.75,'top '+str(int(round((1-upr_pcnt_ecrps[0])*100,0)))+'% flows',transform=ax2.transAxes,ha='right',fontsize='large')
ax2.text(.05,0.95,'j)',transform=ax2.transAxes,ha='left',va='top',fontsize='xx-large',fontweight='bold')
ax2.yaxis.set_ticklabels([])

fig.savefig('./figs/comparison/%s/%s/%s-%s_10panel_cumul-rnk-hist_bse_ecrps_val-lds%s,%s_synvers%s,%s.png' %(loc,site,loc,site,ld,ld2,syn_vers1+vers_out1,syn_vers2+vers_out2),dpi=300,bbox_inches='tight')
